"""
V24 — 1-Month M30 Backtest (Same V22 Strategy, Different Timeframe)
====================================================================
Compares M30 vs M15 resolution to see which timeframe delivers
better V22 results. Uses 30 days of real MT5 data.
Live bot (main_v22.py) is NOT modified — this is standalone.
"""
...
class Tracker:
    def __init__(self): self.bal = 300; self.opens = []; self.losses = 0; self.halt = None; self.daily = 0.0; self.trades = []; self.peak = 300; self.low = 300
    def update(self, px, atr):
        atr = atr or 3.5; cl, rm = [], []
        for p in self.opens:
            e, d, sl, tp, lot = p["e"], p["d"], p["sl"], p["tp"], p["lot"]; pv = lot * 100
            if not p.get("be"):
                if (px - e if d == "BUY" else e - px) >= atr: p["be"] = True; p["sl"] = e
            elif p.get("be"):
                if d == "BUY": ns = px - atr * 0.7; p["sl"] = round(ns, 2) if ns > sl + 0.5 else sl
                else: ns = px + atr * 0.7; p["sl"] = round(ns, 2) if ns < sl - 0.5 else sl
            ...
            if hit:
                self.bal += pnl; self.daily += pnl; self.peak = max(self.peak, self.bal); self.low = min(self.low, self.bal)
...
def run():
    logger.info(f"V24 M30 BACKTEST — {DAYS} days (real MT5 data)")
    ...
    data = {}
    for tf_name, tf_val in [("M5", mt5.TIMEFRAME_M5), ("M15", mt5.TIMEFRAME_M15), ("M30", mt5.TIMEFRAME_M30)]:
        rates = mt5.copy_rates_range("XAUUSD", tf_val, start, end)
        if rates is None or len(rates) == 0:
            mt5.shutdown(); logger.critical(f"No {tf_name} data"); return
    ...
    m5, m15, m30 = data["M5"], data["M15"], data["M30"]
    trk = Tracker()
    strat = GoldScalpingStrategy(); strat._max_trades_per_day = 200; strat._max_open_positions = 10
    vf = GoldVolatilityFilter()
    t0 = time.time(); total = len(m30)

    for i in range(50, total):  # 50-bar warmup for M30
        dt = m30.index[i]; px = float(m30["close"].iloc[i])
        if i % 200 == 0: logger.info(f"  {int(i/total*100)}% | Bal:${trk.bal:.0f} | T:{len(trk.trades)} | Op:{len(trk.opens)}")

        # Daily reset
        if dt.hour == 0: trk.reset()

        # Strategy uses M5 as "M1" substitute, M15 as "M5", M30 as "M15"
        m5w = m5[m5.index <= dt].iloc[-500:]
        m15w = m15[m15.index <= dt].iloc[-500:]
        m30w = m30[max(0, i+1-500):i+1]
        if len(m5w) < 50 or len(m15w) < 50 or len(m30w) < 50: continue

        try: m5i = compute_all_indicators(m5w); m15i = compute_all_indicators(m15w)
        except: continue
        ...
        # Strategy: Use M5 as M1, M15 as M5, M30 as M15 (shift all TFs up one)
        try:
            res = strat.analyze(m1_indicators=m5i, m5_indicators=m15i, m15_indicators=compute_all_indicators(m30w),
                                m1_ohlcv=m5w, m5_ohlcv=m15w, m15_ohlcv=m30w, news_context=None)
        except: continue
        ...
        try:
            vo = vf.analyze(m1_ohlcv=m5w, m5_ohlcv=m15w, m15_ohlcv=m30w,
                            m1_indicators=m5i, m5_indicators=m15i, m15_indicators=compute_all_indicators(m30w))
            if not vo.get("trade_ok", False): continue
        except: continue

        sd = atr * 1.5; td = atr * 3.0
        sl = round(px - sd, 2) if d == "BUY" else round(px + sd, 2)
        tp = round(px + td, 2) if d == "BUY" else round(px - td, 2)
        rp = get_risk_pct(trk.bal); lot = max(0.01, min(trk.bal * (rp/100) / (sd * 100) if sd else 0.01, 50)); lot = round(lot, 2)
        trk.opens.append({"e": px, "tp": tp, "sl": sl, "d": d, "lot": lot, "be": False})

    # Close remaining
    final_px = float(m30["close"].iloc[-1])
    ...
    bp = sum(x["pnl"] for x in t if x["d"] == "BUY"); sp = sum(x["pnl"] for x in t if x["d"] == "SELL")
    ...
    logger.info("\n" + "=" * 70)
    logger.info("V24 M30 BACKTEST RESULTS (30 days)")
    logger.info("=" * 70)
    logger.info(f"  Window: {m30.index[0].date()} to {m30.index[-1].date()}")
    logger.info(f"  M30 candles: {total} | Entry: every 30 min")
    ...
    logger.info("\nM15 vs M30 COMPARISON (same 30-day window, same V22 strategy)")
    logger.info(f"  {'Metric':<22s} {'V23 M15':>15s} {'V24 M30':>15s}")
    logger.info(f"  {'-'*52}")
    logger.info(f"  {'Final Balance':<22s} {'$259':>15s} ${fb:>14,.0f}")
    logger.info(f"  {'Net Profit':<22s} {'-$41':>15s} ${tp:>+14,.0f}")
    logger.info(f"  {'Win Rate':<22s} {'16.7%':>15s} {wr:>14.1f}%")
    logger.info(f"  {'Total Trades':<22s} {'6':>15s} {n:>15}")
    logger.info(f"  {'Lowest Balance':<22s} {'$259':>15s} ${trk.low:>14,.0f}")
    logger.info(f"  {'Profit Factor':<22s} {'0.04':>15s} {pf:>15.2f}")
