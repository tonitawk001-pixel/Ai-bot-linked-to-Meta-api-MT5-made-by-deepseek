"""
V16 — IMPROVED 6-Month Backtest (Real MT5 Data)
================================================

Improvements over V15 based on loss pattern analysis:
  A. Block death-zone hours: 10:00-12:59 UTC and 16:00-16:59 UTC (73-84% loss rates)
  B. Skip Thursdays (-$179 loss day)
  C. Raise minimum score from 30 → 50 (filter weak mean-reversion signals)
  D. Keep max positions at 3 (to maintain 10+ trades/day target)
"""
...
# --- V16 FILTERS ---
DEATH_ZONE_HOURS = {10, 11, 12, 16}  # UTC hours with 73-84% loss rates
SKIP_THURSDAYS = True
MIN_SCORE_THRESHOLD = 50  # Raised from 30
MAX_POSITIONS = 3  # Keep at 3 for volume
...
def run_v16():
    logger.info("=" * 70)
    logger.info("V16 — IMPROVED 6-MONTH XAUUSD BACKTEST (REAL MT5 DATA)")
    logger.info("Improvements: Block death hours (10-12,16 UTC) + Skip Thursdays + MinScore 50")
    ...
    for idx in range(min_warmup, total):
        ...
        # === V16 FILTER 1: Block death-zone hours ===
        if dt.hour in DEATH_ZONE_HOURS:
            blocked_reasons["death_hour"] += 1
            continue

        # === V16 FILTER 2: Skip Thursdays ===
        ...
        # Position closures
        tracker.update_all(price_m15, dt)
    ...
    logger.info("\n" + "=" * 70)
    logger.info("V16 — IMPROVED 6-MONTH XAUUSD BACKTEST RESULTS")
    logger.info("=" * 70)
    ...
    logger.info(f"  ──────────────────────────────────────")
    logger.info(f"  Blocked by death-hour:  {blocked_reasons.get('death_hour', 0)}")
    logger.info(f"  Blocked by Thursday:    {blocked_reasons.get('thursday', 0)}")
    logger.info(f"  Blocked by low score:   {blocked_reasons.get('low_score', 0)}")
    logger.info(f"  Blocked by vol filter:  {blocked_reasons.get('vol_filter', 0)}")
    logger.info("=" * 70)

    # Save trades
    trades_file = os.path.join(os.path.dirname(__file__), "..", "..", "logs", "v16_trades.json")
    ...
    # V15 comparison
    logger.info("\n" + "=" * 70)
    logger.info("V15 vs V16 COMPARISON")
    logger.info("=" * 70)
    logger.info(f"{'Metric':<25s} {'V15':>10s} {'V16':>10s} {'Change':>10s}")
    logger.info("-" * 55)
    logger.info(f"{'Total Trades':<25s} {'504':>10s} {total_trades:>10d} {'':>10s}")
    logger.info(f"{'Win Rate':<25s} {'42.3%':>10s} {win_rate:>9.1f}% {'':>10s}")
    logger.info(f"{'Final Balance':<25s} {'$1138.44':>10s} ${final_balance:>9.2f} {'':>10s}")
    logger.info(f"{'Total P/L':<25s} {'$+838.44':>10s} ${total_pnl:>+9.2f} {'':>10s}")
    logger.info(f"{'Profit Factor':<25s} {'1.32':>10s} {profit_factor:>10.2f} {'':>10s}")
    logger.info(f"{'Return %':<25s} {'+279.5%':>10s} {(total_pnl / INITIAL_BALANCE * 100):>+9.1f}% {'':>10s}")
    logger.info(f"{'Avg Trades/Day':<25s} {'12.3':>10s} {avg_trades_per_day:>10.1f} {'':>10s}")
    logger.info("=" * 70)

    return {"total_trades": total_trades, "win_rate": round(win_rate, 1), "total_pnl": round(total_pnl, 2),
            "final_balance": round(final_balance, 2), "profit_factor": round(profit_factor, 2),
            "max_drawdown_pct": round(max_dd, 1), "max_consecutive_losses": max_consec}
