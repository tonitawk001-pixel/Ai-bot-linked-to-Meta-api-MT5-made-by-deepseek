"""
Gold Scalping V10 — 90%+ WIN RATE, MORE TRADES
===============================================

Improvements from V9:
  - Max positions: 5 → 10 (more concurrent trades)
  - TP: 3.0x ATR → 2.5x ATR (faster profit locking)
  - Score 80-89, BUY only, bullish bias, 1.0 lot
  - No max_hold, SL=10x ATR
"""
...
def run_v10(days=7, seed=42):
    ...
    print(f"""
{'='*75}
  V10 — IMPROVED 90%+ WIN RATE (7-Day XAUUSD)
{'='*75}

  TIMEFRAME: M15 | LOT: 1.0 | TP: 2.5x ATR | SL: 10x ATR
  SCORE: 80-89 | BUY ONLY | BULLISH BIAS | MAX POS: 10
...
if __name__ == "__main__":
    ...
    p.add_argument("--output", default="trading_bot/backtest/v10_report.json")
    a = p.parse_args()
    ...
    if a.multi_seed:
        ...
        print(f"{'='*50}")
        for km in ["total_trades", "win_rate", "profit_factor", "total_pnl", "return_pct", "max_dd_pct", "avg_trades_day"]:
            if f"{km}_avg" in agg:
                print(f"  {km:<25} {agg[f'{km}_avg']:>8.2f} ± {agg[f'{km}_std']:>6.2f}")
        report = {"multi": agg}
    else:
        report = run_v10(days=a.days, seed=a.seed)
