"""
V23 — 1-Minute Resolution Backtest (Simulates Live Bot)
=========================================================
Walks M1 candles (every minute), checks SL/TP at each bar,
and runs the same V22 strategy used by the live bot.
Tests 30 days of recent data.
"""
...
def run():
    logger.info(f"V23 M1-RESOLUTION BACKTEST — {DAYS} days")
    ...
    for tf_name, tf_val in [("M1", mt5.TIMEFRAME_M1), ("M5", mt5.TIMEFRAME_M5), ("M15", mt5.TIMEFRAME_M15)]:
        ...
        if rates is None or len(rates) == 0:
            if tf_name == "M1": logger.warning("M1 unavailable — using M5 for M1"); rates = mt5.copy_rates_range("XAUUSD", mt5.TIMEFRAME_M5, start, end)
            if rates is None or len(rates) == 0: mt5.shutdown(); logger.critical(f"No {tf_name}"); return
    ...
    t0 = time.time(); total = len(m1)

    for i in range(200, total):  # 200-bar warmup
        dt = m1.index[i]; px = float(m1["close"].iloc[i])
        if i % 5000 == 0: logger.info(f"  {int((i-200)/total*100)}% | Bal:${trk.bal:.0f} | T:{len(trk.trades)} | Op:{len(trk.opens)}")

        # Daily reset
        if dt.hour == 0 and dt.minute == 0: trk.reset()

        # Get windows
        m1w = m1[max(0, i+1-500):i+1]
        ...
        # Strategy on M1 close (simulate live)
        try:
            res = strat.analyze(m1_indicators=m5i, m5_indicators=m5i, m15_indicators=m15i,
    ...
    # Close remaining
    trk.update(float(m1["close"].iloc[-1]), atr); trk.bal += sum(
        ((float(m1["close"].iloc[-1]) - p["e"]) if p["d"] == "BUY" else (p["e"] - float(m1["close"].iloc[-1]))) * p["lot"] * 100
        for p in trk.opens)
    trk.opens = []
    ...
    logger.info("\n" + "=" * 70)
    logger.info("V23 M1-RESOLUTION RESULTS (30 days)")
    logger.info("=" * 70)
    logger.info(f"  Window: {m1.index[0].date()} to {m1.index[-1].date()}")
    logger.info(f"  M1 candles: {total} | Simulated at 1-min intervals")
    ...
    logger.info("\nCOMPARISON: M15 vs M1 Resolution")
    logger.info(f"  V22 M15 6-mo W1: $300 -> $70,530 | WR 46.2% | 1839 trades")
    logger.info(f"  V23 M1  1-mo   : ${INITIAL_BALANCE:.0f} -> ${fb:.0f} | WR {wr:.1f}% | {n} trades")
    logger.info(f"  Note: different time windows — compare WR/PF for resolution effect")
